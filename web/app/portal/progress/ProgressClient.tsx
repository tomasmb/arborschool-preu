...
function computeProjection(
  ...
  for (let week = 1; week <= MAX_PROJECTION_WEEKS; week++) {
    ...
    const lockedPerTest = Math.max(0, PAES_TOTAL_QUESTIONS - unlockedPerTest);
    ...
    // Floor at current score: the student already demonstrated this level,
    // so the projection should only go up from here as atoms are mastered.
    const rawMid = getPaesScore(Math.round(expectedCorrect));
    const projectedMid = Math.max(meta.currentScore, rawMid);
    ...
    >
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          {isPreFirstTest ? (
            <LockedProjectionSection currentScore={data.currentScore} />
          ) : (
            <>
              {projection && (
                <ScoreJourneyChart
