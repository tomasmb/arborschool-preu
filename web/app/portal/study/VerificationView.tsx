...
type Controller = ReturnType<typeof useVerificationController>;
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-gray-600 max-w-md mx-auto">
        No tienes conceptos pendientes de verificación. Sigue aprendiendo y
        tomaremos nota de cualquier ajuste necesario.
      </p>
...
function VerificationQuestionCard({ ctrl }: { ctrl: Controller }) {
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <p className="text-xs uppercase tracking-wide text-gray-500 font-medium">
              Verificación {ctrl.currentIndex + 1} de {ctrl.totalItems}
            </p>
      ...
      {hasFeedback && (
        ...
        >
          ...
          <p className="text-xs text-center text-gray-600">
            {ctrl.answerResult!.isCorrect
              ? "Concepto confirmado — sigue contando como dominado."
              : "Vacío detectado — lo agregaremos a tu camino de aprendizaje."}
          </p>
        </section>
      )}
...
function VerificationResultPanel({ ctrl }: { ctrl: Controller }) {
  const summary = ctrl.resultSummary;
  if (!summary) return null;

  const allRestored = summary.downgraded === 0;
  const total = summary.restored + summary.downgraded;
  ...
  return (
    ...
    >
      <div className="text-center space-y-2">
        ...
        >
          {allRestored ? "Todo confirmado" : "Verificación completada"}
        </div>
        <h2 className="text-2xl sm:text-3xl font-serif font-bold text-primary">
          {summary.restored} de {total} confirmados
        </h2>
        {summary.downgraded > 0 && (
          <p className="text-sm text-gray-500">
            {summary.downgraded}{" "}
            {summary.downgraded === 1
              ? "concepto requiere repaso"
              : "conceptos requieren repaso"}
          </p>
        )}
      </div>

      {summary.prereqScans.length > 0 && (
        <div
          className="rounded-xl bg-amber-50 border border-amber-200
            p-4 text-center space-y-2"
        >
          <p className="text-sm text-amber-700">
            Detectamos vacíos en conceptos previos. Se agregaron a tu camino de
            aprendizaje.
          </p>
        </div>
      )}
...
export function VerificationView({ ctrl }: { ctrl: Controller }) {
  if (ctrl.phase === "loading") return <QuestionSkeleton />;
  if (ctrl.phase === "empty") return <EmptyVerificationState />;
  if (ctrl.phase === "error") {
    return <ErrorStatePanel message={ctrl.error ?? "Error en verificación"} />;
  }
  if (ctrl.phase === "result") {
    return <VerificationResultPanel ctrl={ctrl} />;
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500">
              Confirmemos algunos conceptos de tu último test
            </p>
          </div>
      ...
      <div key={ctrl.currentItem?.responseId} className="animate-fade-in-up">
        <VerificationQuestionCard ctrl={ctrl} />
      </div>
    </div>
  );
}
