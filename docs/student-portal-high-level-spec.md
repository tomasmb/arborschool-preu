...
- Makes post-diagnostic value immediate (mini-clase and mission), not deferred.
...
- `activation_ready`: diagnostic completed, ready for first mission/mini-clase
...
- Show immediate handoff with two actions:
  - Primary: "Comenzar mini-clase de hoy"
...
- `/portal/study` starts mini-clase quickly with minimal preamble
...
- Keep vocabulary stable across product and email:
  - `meta` -> `diagnóstico` -> `mini-clase` -> `misión semanal`
...
- Diagnostic completion summary with direct CTA to first mini-clase

2. Lifecycle nudges (state-driven)

- Planning started but not saved
- Diagnostic started but not completed
- Diagnostic completed but first mini-clase not started
...
- Trigger: diagnostic completed, no mini-clase started
- Send: "start first mini-clase" email with effort and expected impact context
- Stop condition: first mini-clase started
...
- Hero must state immediate value and time expectation (goal + diagnostic + first
  mini-clase), not future availability.
- "How it works" must map exactly to live flow:
  - Define target
  - Complete diagnostic
  - Start first mini-clase
...
- Meet WCAG 2.2 AA on all core journey screens:
  ...
  study mini-clase, and email templates.
...
- Track every stage transition and drop-off point:
  ...
  -> first mini-clase started -> weekly active.
...
- Insert short SR blocks after every 2-3 newly mastered atoms OR when due by session count (session-based intervals: sessionsSinceLastReview >= reviewIntervalSessions)
