...
13. Email CTA deep links now carry explicit intent context (`source=email`,
...
1. Funnel dashboard/report QA for milestone segmentation by entry/journey state
2. CI-enforced accessibility/performance gates
...
1. `/portal` -> redirect to `/auth/signin` with callback preserving `/portal`
2. `/api/student/goals` -> `401`
3. `/diagnostico` -> redirect to `/auth/signin` with callback preserving
   `/diagnostico`
...
### A9. Callback + Stale-Link Reliability Contract
...
1. Auth callback URLs preserve deep-link query parameters
